...
**NEW (EHT-314): RLS & Compliance Owner**
...
cat scripts/rls-phase4-final-validation.sql | docker exec -i EHTravel-postgres-1 psql -U EHTravel_app_user -d EHTravel_dev && npm audit --audit-level=high && yarn lint && echo "SECURITY SUCCESS" || echo "SECURITY FAILED"
```
...
grep -A 5 "Security:" specs/EHT-XXX-{feature}-spec.md
```
...
```bash
# Automated RLS check
cat scripts/rls-phase4-final-validation.sql | docker exec -i EHTravel-postgres-1 psql -U EHTravel_app_user -d EHTravel_dev
