...
tools: [Read, Write, Edit, Bash, Grep, Glob, mcp__linear__*]
model: opus
---
...
### 0. Check Pattern Library FIRST (MANDATORY - EHT-300)
...
```bash
# Create spec file for EHT-XXX
cp specs/spec_template.md specs/EHT-XXX-{description}-spec.md
```

#### Step 2: Extract from User Story

**From Linear ticket (EHT-XXX)**:
...
ls specs/EHT-*-spec.md | grep "similar_feature"
...
- Implement [feature] as specified in EHT-XXX
...
- How it fits into existing EHTravel architecture
- Components affected
...
- Internal dependencies (EHTravel components)
...
From spec, add subtasks to EHT-XXX:
