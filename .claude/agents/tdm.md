...
tools: [Read, Bash, mcp__linear__*, mcp__confluence__*]
model: opus
---
...
git log --oneline -10 | grep -E "EHT-[0-9]+" && echo "LINEAR TRACKING SUCCESS"
```
...
- Database schema changes → ARCHitect (ola1145)
...
### When to Escalate to ARCHitect (ola1145)
...
- PR #123: [EHT-XXX] - [Status]
- PR #124: [EHT-XXX] - [Status]
...
```bash
# Agent 1: FE Developer on EHT-123
# Agent 2: BE Developer on EHT-124
...
```bash
# Agent 1: DE creates migration (EHT-125)
# Agent 2: BE implements API (EHT-126) - depends on EHT-125
# TDM ensures EHT-125 merged before EHT-126 starts
```
