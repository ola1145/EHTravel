...
**QA Reports**: `/docs/agent-outputs/qa-validations/EHT-{number}-qa-validation.md`

**Naming Convention**: `EHT-{number}-qa-validation.md`
...
1. **Read spec** → `cat specs/EHT-XXX-{feature}-spec.md`
...
## Pattern Execution Workflow (EHT-300)
...
grep -A 10 "Testing Strategy" specs/EHT-XXX-{feature}-spec.md

# Find pattern references
grep -A 3 "Pattern:" specs/EHT-XXX-{feature}-spec.md
```
...
3. **Linear Evidence Posted**
   - [ ] QA report created at `/docs/agent-outputs/qa-validations/EHT-{number}-qa-validation.md`
   - [ ] Final verdict posted to Linear comments via `mcp__linear__create_comment`

4. **Handoff Statement**
   > "QAS validation complete for EHT-XXX. All criteria PASSED. Evidence posted to Linear. Approved for RTE."

**Or if BLOCKED:**

> "QAS validation BLOCKED for EHT-XXX. Issues: [list]. Returning to [implementer/role] for fixes."
