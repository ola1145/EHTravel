...
### NEW (EHT-314): Production Deployment Owner
...
git log --oneline -10 | grep -E "EHT-[0-9]+" && echo "COMMIT FORMAT SUCCESS"
...
cat specs/EHT-XXX-{feature}-spec.md | grep -A 30 "Pull Request Template"

# Extract logical commits
grep -r "Logical Commits|git commit" specs/EHT-XXX-spec.md

# Get demo script for validation
grep -r "Demo Script" specs/EHT-XXX-spec.md
```

### 5. Review Documentation

- `CONTRIBUTING.md` - Complete workflow (MANDATORY)
- `specs/EHT-XXX-{feature}-spec.md` - Implementation spec with PR template
...
```bash
cat specs/EHT-XXX-{feature}-spec.md
```
...
git branch --show-current | grep -E "^EHT-[0-9]+-" && echo "✅ Branch name valid"

# 2. Verify commit message format
git log --oneline -1 | grep -E "^[a-z]+(\([a-z]+\))?: .+ \[EHT-[0-9]+\]" && echo "✅ Commit format valid"
...
- [ ] Branch name: `EHT-{number}-{description}` ✅
- [ ] Commits follow SAFe format: `type(scope): description [EHT-XXX]` ✅
...
gh pr create --title "feat(scope): description [EHT-XXX]" --body "$(cat <<'EOF'
## 📋 Summary

Implements [feature/fix] as specified in Linear ticket EHT-XXX.

**Linear Ticket**: https://linear.app/ehtravel/issue/EHT-XXX
...
3. Select base: `dev` and compare: `EHT-{number}-{description}`
...
git commit -m "fix(ci): resolve test failure [EHT-XXX]"
...
git add .
git commit -m "refactor(scope): address PR feedback [EHT-XXX]"
...
**You do NOT merge** - Ola McCartney (or designated HITL) is final merge authority.
...
> "PR #XXX for EHT-YYY is Ready for HITL Review. All CI green, reviews complete, evidence attached. Awaiting final merge approval from Ola McCartney."

**Notify Ola McCartney** and wait for merge.
...
git log --oneline -5 | grep "EHT-XXX"

# Linear ticket auto-sync:
# - Tickets referenced in commit messages (e.g., [EHT-XXX]) auto-move to Done
...
- SAFe commit format: `type(scope): description [EHT-XXX]`
- Branch naming: `EHT-{number}-{description}`
...
- PR Number: #XXX
- Title: feat(scope): description [EHT-XXX]
- Base: dev
- Compare: EHT-XXX-description
...
# 2. Rebase and push
git fetch origin && git rebase origin/dev
git push --force-with-lease origin EHT-123-feature

# 3. Create PR
gh pr create --title "feat(feature): implement feature [EHT-123]" --web

# 4. Monitor CI
gh pr checks

# 5. Handoff to HITL (RTE does NOT merge)
# Notify Ola McCartney: "PR #XXX ready for HITL review"
...
git checkout -b EHT-999-hotfix-critical-bug
...
gh pr create --base main --title "fix(critical): resolve security issue [EHT-999]"

# 4. Handoff to HITL for emergency merge
# Notify Ola McCartney: "Emergency PR ready - blocks production"
# RTE work ends here - Scott handles merge via GitHub
...
```bash
# Agent A (FE): EHT-123-ui-component (depends on EHT-124)
# Agent B (BE): EHT-124-api-endpoint (must merge first)

# RTE coordinates (but does NOT merge):
# 1. Notify HITL: "EHT-124 ready, blocks EHT-123"
# Wait for Scott to merge EHT-124 via GitHub

# 2. After HITL merges EHT-124, RTE rebases EHT-123
git checkout EHT-123-ui-component
git fetch origin && git rebase origin/dev
git push --force-with-lease

# 3. Notify HITL: "EHT-123 ready after EHT-124 merged"
# RTE work ends here - Scott handles merge via GitHub
```
...
- **No Merge**: You prepare for merge, HITL (Ola McCartney) does the merge
...
4. **Handoff Statement**
   > "PR #XXX for EHT-YYY is Ready for HITL Review. CI green, reviews complete, evidence attached."
