...
# └─────────────────────────────────────────────────────────────────────┘
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "cd ehtravel-org && {{DEPLOY_SCRIPT}}"
...
# Health check
curl -s http://<your-staging-host>:{{STAGING_PORT}}/api/health | jq

# Container status
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "docker ps --filter name={{CONTAINER_NAME}}"
```
...
| `<your-staging-host>`    | Staging server hostname/IP    | `staging.example.com`       |
