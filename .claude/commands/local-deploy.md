...
| ------------------------- | ----------- | ---------------------------- | ---------------------------------------------------------------------------- |
| **Development** (default) | (none)      | `EHTravel-dev-app`     | Active development with hot-reload (STANDARD port 3000)                      |
| **Staging**               | `--staging` | `EHTravel-staging-app` | Production-like, self-contained, survives git operations (port 3001)         |
| **Both**                  | `--both`    | Both containers              | Run dev and staging simultaneously (EHT-400/EHT-401) |
...
docker ps --filter name=EHTravel-staging-app --format '{{.Names}}\t{{.Status}}'

# Check for dev container (EHT-400: updated container name)
docker ps --filter name=EHTravel-dev-app --format '{{.Names}}\t{{.Status}}'
```

Get current commit SHA:

```bash
# Try staging first, then dev (EHT-400: updated container names)
docker inspect EHTravel-staging-app 2>/dev/null | grep 'org.opencontainers.image.revision' | cut -d'"' -f4 || \
docker inspect EHTravel-dev-app 2>/dev/null | grep 'org.opencontainers.image.revision' | cut -d'"' -f4
```
...
```bash
# For dev mode (STANDARD port 3000, EHT-401)
docker ps --filter name=EHTravel-dev --format 'table {{.Names}}\t{{.Status}}'

# For staging mode (port 3001)
docker ps --filter name=EHTravel-staging --format 'table {{.Names}}\t{{.Status}}'
```
...
```bash
# Staging
docker inspect EHTravel-staging-app | grep 'org.opencontainers.image.revision' | cut -d'"' -f4

# Dev (EHT-400: updated container name)
docker inspect EHTravel-dev-app | grep 'org.opencontainers.image.revision' | cut -d'"' -f4
```
...
```bash
# Staging (EHT-400: use container name)
docker logs EHTravel-staging-app --tail 50

# Dev (EHT-400: use container name or script)
docker logs EHTravel-dev-app --tail 50
# Or use script
...
Current:  e9722d4 - style(docs): apply markdown linting fixes [EHT-347]
Status:   Up 7 hours (healthy)

Target:   3a49b85 - feat(ci): add Slack notifications [EHT-350]
...
Running:  3a49b85 - feat(ci): add Slack notifications [EHT-350]
Status:   Healthy
...
## Running Both Modes Simultaneously (EHT-400/EHT-401)
...
```bash
docker ps --filter name=EHTravel --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'
```
