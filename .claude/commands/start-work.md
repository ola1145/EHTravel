...
argument-hint: [EHT-number]
allowed-tools: [Read, Write, Edit, Bash, Grep, Glob, mcp__linear__*]
---
...
3. **Branch Naming**
   - Format: `EHT-{number}-{short-description}`
   - Must start with EHT- and ticket number
...
5. **Create Feature Branch**
   - Create branch: `git checkout -b EHT-{number}-{description}`
...
- Use as ticket number (e.g., `/start-work 347` → EHT-347)
