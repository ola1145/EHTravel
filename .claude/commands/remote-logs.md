...
# └─────────────────────────────────────────────────────────┘
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "cd ehtravel-org && ./scripts/dev-docker.sh logs --tail 100"
```
...
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "docker logs {{APP_CONTAINER_DEV}} --tail 100"

# App logs (staging-mode - port 3001)
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "docker logs {{APP_CONTAINER_STAGING}} --tail 100"

# PostgreSQL logs (staging)
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "docker logs {{DB_CONTAINER_STAGING}} --tail 100"

# PostgreSQL logs (dev)
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "docker logs {{DB_CONTAINER_DEV}} --tail 100"

# Redis logs (staging)
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "docker logs {{REDIS_CONTAINER_STAGING}} --tail 100"

# Redis logs (dev)
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "docker logs {{REDIS_CONTAINER_DEV}} --tail 100"
```
...
```bash
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "cd ehtravel-org && ./scripts/dev-docker.sh logs --follow"
```
...
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "docker logs {{APP_CONTAINER_STAGING}} 2>&1 | grep -i 'search-term'"
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "docker logs {{APP_CONTAINER_DEV}} 2>&1 | grep -i 'search-term'"
```
...
ping <your-staging-host>

# Or check via VPN/Tailscale
tailscale status | grep <your-staging-host>
```
