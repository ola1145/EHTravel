...
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> \
  "docker ps --filter name=EHTravel --format 'table {{.Names}}\t{{.Status}}\t{{.State}}'"
```
...
```bash
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "df -h / | tail -1"
```

Check Docker disk usage:

```bash
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "docker system df"
```
...
curl -s -w '\nHTTP Status: %{http_code}\nResponse Time: %{time_total}s\n' \
  http://<your-staging-host>:{{APP_PORT}}/api/health
```
...
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> \
  "docker exec ehtravel-db pg_isready -U {{DB_USER}}"
```
...
EHTravel-app            ✅ Healthy  Up 2 hours
EHTravel-postgres       ✅ Healthy  Up 2 hours
EHTravel-redis          ✅ Healthy  Up 2 hours
...
EHTravel-app          2.45%    312MB / 2GB  (15%)
EHTravel-postgres     0.12%    45MB / 2GB   (2%)
EHTravel-redis        0.08%    8MB / 2GB    (0%)
...
| `ehtravel-db`    | Database container name   | `myapp-postgres`            |
