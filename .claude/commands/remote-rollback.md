...
# └─────────────────────────────────────────────────────────┘
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "docker inspect {{APP_CONTAINER_STAGING}} 2>/dev/null | grep 'org.opencontainers.image.revision' | cut -d'\"' -f4 || docker inspect {{APP_CONTAINER_DEV}} | grep 'org.opencontainers.image.revision' | cut -d'\"' -f4"
```

Check current status:

```bash
curl -s http://<your-staging-host>:{{DEV_PORT}}/api/health 2>/dev/null || echo "Health check failed"
```
...
```bash
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "docker images {{REGISTRY}}/{{IMAGE_NAME}} --format 'table {{.Tag}}\t{{.ID}}\t{{.CreatedAt}}'"
```
...
```bash
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "docker pull {{REGISTRY}}/{{IMAGE_NAME}}:{{TAG_PREFIX}}-{target-sha}"
```
...
```bash
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "cd ehtravel-org && cp {{COMPOSE_FILE}} {{COMPOSE_FILE}}.backup.$(date +%Y%m%d_%H%M%S)"
```
...
```bash
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "cd ehtravel-org && sed -i 's|{{REGISTRY}}/{{IMAGE_NAME}}:latest|{{REGISTRY}}/{{IMAGE_NAME}}:{{TAG_PREFIX}}-{target-sha}|g' {{COMPOSE_FILE}}"
```
...
```bash
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "cd ehtravel-org && {{DOCKER_SCRIPT}} restart"
```

Monitor startup:

```bash
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "cd ehtravel-org && {{DOCKER_SCRIPT}} logs --tail 50"
```
...
```bash
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "docker ps --filter name={{CONTAINER_PREFIX}}"
```

Verify health endpoint:

```bash
curl -s http://<your-staging-host>:{{DEV_PORT}}/api/health | jq
```
...
Status:   ✅ Healthy
URL:      http://<your-staging-host>:{{DEV_PORT}}
Duration: 1m 23s
...
ssh -i ~/.ssh/id_ed25519 deploy@<your-staging-host> "cd ehtravel-org && sed -i 's|{{REGISTRY}}/{{IMAGE_NAME}}:{{TAG_PREFIX}}-[a-f0-9]*|{{REGISTRY}}/{{IMAGE_NAME}}:latest|g' {{COMPOSE_FILE}}"
...
| `ehtravel-org`          | Project directory on remote      | `~/app`                     |
...
| `EHT`         | Linear/Jira ticket prefix        | `WOR`, `PROJ`, `FEAT`       |
