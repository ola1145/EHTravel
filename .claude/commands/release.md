...
> **📋 TEMPLATE**: This command uses `main`, `EHT`, and `ola1145/ehtravel-org` placeholders. Replace with your project values.
...
gh pr merge <NUMBER> --squash --subject "type(scope): description [EHT-XXX]"
```
...
gh api repos/ola1145/ehtravel-org/tags --jq '.[0:5] | .[].name'
```
...
gh api repos/ola1145/ehtravel-org/branches --jq '.[].name' | grep -v '^main$\|^dev$'
```

For each stale branch from a merged PR:

```bash
gh api -X DELETE repos/ola1145/ehtravel-org/git/refs/heads/<BRANCH_NAME>
```
...
echo "=== Remote ===" && gh api repos/ola1145/ehtravel-org/branches --jq '.[].name'
...
| `EHT` | Linear/Jira ticket prefix | `WOR`, `SCA`, `PROJ` |
| `ola1145` | GitHub organization | `bybren-llc` |
| `ehtravel-org` | GitHub repository name | `safe-agentic-workflow` |
