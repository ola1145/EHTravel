...
CREATE POLICY user_data_user_select ON user_data
  FOR SELECT TO EHTravel_app_user
...
-- Grant permissions
GRANT SELECT, INSERT, UPDATE ON user_data TO EHTravel_app_user;
```
...
CREATE POLICY {table}_user_insert ON {table}
  FOR INSERT TO EHTravel_app_user
...
- [ ] @ola1145 must be present (MANDATORY)
